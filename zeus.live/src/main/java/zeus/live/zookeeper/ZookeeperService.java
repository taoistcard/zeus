...
public class ZookeeperService implements IZkDataListener, IZkStateListener {
	...
	IActiveLiveServerListener activeLiveServerListener;
	 ...
	 */
	public interface IActiveLiveServerListener {
		public void onActiveLiveServerChanged(List<ServerInfo> serverInfos);
	}
	...
	public void registerServer(ServerInfo serverInfo) {
		...
		String key = this.serverInfo.getServerKey();
	...
	public void registerFreeServer() {
		deleteNode(nodePath);
		nodeName = String.format("%s:[innet]%s:[dirtcp]%s:[dws]%s:%s", serverInfo.getHost(),
				serverInfo.getConnectorPort(), serverInfo.getDirectTcpPort(), serverInfo.getDirectWebsocketPort(),
				StringUtils.createUUID());
		nodePath = freeRoomServer + "/" + nodeName;
		
		serverInfo.setNodePath(nodePath);
	...
	public void registerActiveServer() {
		if (nodePath != null) {
			deleteNode(nodePath);
		}
		...
		nodeName = String.format("%s:%s:[innet]%s:[dirtcp]%s:[dws]%s", serverInfo.getServerKey(), serverInfo.getHost(),
				serverInfo.getConnectorPort(), serverInfo.getDirectTcpPort(), serverInfo.getDirectWebsocketPort());
		nodePath = activeRoomServer + "/" + nodeName;
		serverInfo.setNodePath(nodePath);
		nodeData = JSON.toJSONString(serverInfo);
		if (zkUtil.exists(nodePath)) {
			deleteNode(nodePath);
